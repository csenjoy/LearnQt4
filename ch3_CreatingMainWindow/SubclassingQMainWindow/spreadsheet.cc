...
void Spreadsheet::recalculate() {
  for (int row = 0; row < RowCount; ++row) {
    for (int column = 0; column < ColumnCount; ++column) {
      if (cell(row, column)) {
        //The next time QTableWidget calls text() on a Cell, the value will be recalculated
        cell(row, column)->setDirty();
      }
    }
  }
  //To repaint the whole spreadsheet. The repaint code int QTableWidget then calls text() on each visible cell
  viewport()->update();
}
...
//return true if the first row is less then the second row
