    ...
    : QWidget(parent)
{
    ...
    zoomInButton->setIcon(QIcon(tr(":/images/zoomin.png")));
...
void Plotter::mouseReleaseEvent(QMouseEvent *event) 
{
    if ((event->button() == Qt::LeftButton) && rubberBandIsShown) {
        ...
        PlotSettings preSetting = zoomStack[curZoom];
        PlotSettings settings;
        double dx = preSetting.spanX() / (width() - 2 * Margin);
        double dy = preSetting.spanY() / (height() - 2 * Margin);
        settings.minX = preSetting.minX + dx * rect.left();
        settings.maxX = preSetting.minX + dx * rect.right();
        settings.minY = preSetting.maxY - dy * rect.bottom();
        settings.maxY = preSetting.maxY - dy * rect.top();
        settings.adjust();
...
void Plotter::drawCurves(QPainter *painter) 
{
    ...
    while (i.hasNext()) {
        ...
        for (int j = 0; j < data.count(); ++j) {
            ...
            polyine[j] = QPointF(x, y);
        }
        painter->setPen(colorForIds[uint(id) % 6]);
        painter->drawPolyline(polyine);
    }
}